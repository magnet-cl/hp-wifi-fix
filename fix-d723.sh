...
# For the HP notebooks the physical antenna is connected to the second connector (ant_sel=2)
# The second parameter disables firmware power saving to avoid random disconnections (fwlps=0)
echo "options rtl8723de ant_sel=2 fwlps=0" | sudo tee /etc/modprobe.d/rtl8723de.conf

# Apply the patch now, without reboot, unload existing module
sudo modprobe -r rtl8723de
# Load the new one
sudo modprobe rtl8723de ant_sel=2 fwlps=0
